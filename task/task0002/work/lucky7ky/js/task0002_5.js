var drag = {
    ...
    init: function() {
              ...
              for(var i = 0; i < this.boxes.length; i++) {
                  this.boxes[i].onmousedown = function(evt) { // 给每个长方形box添加onmousedown事件
                      ...
                      document.onmousemove = function (evt) {// 添加onmousemove 事件
                          ...
                          drag.boxes[drag.config.n].style.left = drag.config.left + "px";//实现box随鼠标移动
                          drag.boxes[drag.config.n].style.top = drag.config.top  + "px";
                          drag.boxes[drag.config.n].style.cursor = "move";
                          drag.boxes[drag.config.n].style.zIndex = "1";
    ...
    mouseUp:function () {
                drag.boxes[drag.config.n].onmouseup = function () { // 添加onmouseup事件
                    if (drag.config.left >= 0 && drag.config.left <= 400 && drag.config.top >=0 && drag.config.top <= 550) {//判断移动过程中长方形box是否超过class为content的div的边界
                        ...
                        drag.compactBoxes(drag.containerLeft);//对左边容器的box位置进行处理，使box位置依次紧凑
                        drag.compactBoxes(drag.containerRight);//对右边容器的box位置进行处理，使box位置依次紧凑
                        for (var i = 0; i < drag.boxes.length; i++) {
                            drag.boxes[i].style.zIndex = "0";
                        }
    ...
    active:function () {
               ...
               if(drag.config.left <= 200) { //判断box的位置 决定加入左边还是右边容器  
                   drag.boxes[drag.config.n].style.left = 51 + "px";//将box添加到左边容器 （50px的padding+1px的border）
                   if (drag.boxes[drag.config.n].parentNode != this.containerLeft) { //判断box是否已是左边容器的子元素
                       drag.boxes[drag.config.n].style.top = 51 + 50*insideLeftNumber + "px";//当box来自右边容器，计算box添加到左边容器的top位置(box高度为50px) 
                       this.containerLeft.appendChild(drag.boxes[drag.config.n]);//相应修改DOM
                   } else {
                       drag.boxes[drag.config.n].style.top = 51 + 50*insideLeftNumber-50 + "px";//当box原属于左边容器，释放时回归原位
                   }
               } else { 
                   drag.boxes[drag.config.n].style.left = 349 + "px";//将box添加到右边容器
                   if (drag.boxes[drag.config.n].parentNode != this.containerRight) {//判断box是否已是右边容器的子元素
                       drag.boxes[drag.config.n].style.top = 51 + 50*insideRightNumber + "px";//计算top 添加到其他box的下面
                       this.containerRight.appendChild(drag.boxes[drag.config.n]);//相应修改DOM
                   } else {
                       drag.boxes[drag.config.n].style.top = 51 + 50*insideRightNumber-50 + "px";//当box原属于右边容器，释放时回归原位
                   }
               }

           },
    compactBoxes:function (container){
